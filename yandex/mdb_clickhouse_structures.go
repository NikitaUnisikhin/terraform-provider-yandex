...
func flattenClickHouseUserSettings(settings *clickhouse.UserSettings) map[string]interface{} {
	...
	if settings.LogQueryThreads != nil {
		result["log_query_threads"] = settings.LogQueryThreads.Value
	}
	if settings.MaxInsertThreads != nil {
		result["max_insert_threads"] = settings.MaxInsertThreads.Value
	}
	if settings.UseHedgedRequests != nil {
		result["use_hedged_requests"] = settings.UseHedgedRequests.Value
	}
	...
	if settings.PreferLocalhostReplica != nil {
		result["prefer_localhost_replica"] = settings.PreferLocalhostReplica.Value
	}
